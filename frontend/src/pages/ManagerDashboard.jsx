...
const BillingSession = ({ table, menuItems, onClose, onUpdate, notify }) => {
    ...
    const handlePrintKOT = async () => {
        ...
        const newItemsToPrint = items
            .filter(i => i.dbQty > i.initialDBQty)
            .map(i => ({
                name: i.name,
                quantity: parseFloat((i.dbQty - i.initialDBQty).toFixed(1)), 
                special_instruction: i.note
            }));

        const isFreshTable = !table.active_order_id; 

        try {
            const oid = await syncToDB();
            // If table was cleared (null oid), stop
            if (!oid) {
            ...
            if (newItemsToPrint.length > 0) {
                const steward = localStorage.getItem('username') || 'Admin';
                await api.post('/print/kot', {
                    table_no: table.table_no,
                    steward,
                    items: newItemsToPrint,
                    is_running: !isFreshTable, 
                    table_id: table.id
                });
                notify("KOT Sent");
            } else {
                notify("Nothing new to print");
            }
